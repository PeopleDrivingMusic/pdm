...
debt from git rather than from a marker left by the previous session: it compares
`HEAD` against the `head:` sha recorded by the newest retro for the current
branch, and reports the number of commits in between. If no retro exists for the
branch yet, it counts commits since the default branch instead. The model then
offers to close the debt while `git log` and the diff are still fresh. This
survives an abruptly terminated session, which matters on a machine that BSODs
several times a day.
