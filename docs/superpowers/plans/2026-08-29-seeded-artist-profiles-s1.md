...
- Run `yarn check` after changes; it must stay at **0 errors**.
...
Run: `yarn check`
Expected: **0 errors.** Every existing reader of `artist.userId` is already null-tolerant (`policy.ts:57,63,73` use `?? null`; `access.ts:86` narrows; `+page.server.ts:14` and `api/music/[id]:24` are `===` comparisons), so a non-zero count means something unexpected — stop and report it rather than patching.
...
Run: `yarn test:unit --run src/lib/server/catalog-source/adapters/AudiusAdapter.spec.ts`
Expected: PASS, 20 tests (the `it.each` block counts as 6).
...
**If the script fails to start with an unresolved `$app/environment`**, do not fight the config — add a minimal `vite.config.script.ts` without the `sveltekit()` plugin and run `vite-node -c vite.config.script.ts`. Report which was needed.
...
Expected: PASS, **61 tests** (7 + 20 + 16 + 18).
...
Expected: passes the 90% thresholds. This runs the **whole** `server` project, so a failure may be pre-existing — check whether the uncovered lines are yours before changing anything. If `AudiusAdapter.ts` is short on branch coverage, add fixture rows that exercise the null arms rather than lowering the threshold.

```bash
yarn check   # expected: 0 errors
...
- `yarn test:unit --run` (61 new tests), `yarn test:coverage`, `yarn check` and `yarn lint` all pass.
