...
const loggingHandle: Handle = async ({ event, resolve }) => {
	...
	try {
		...
		MetricsCollector.recordHttpRequest(event.request.method, route, response.status, duration);

		logger.info(
			`Request completed: ${event.request.method} ${event.url.pathname} - ${response.status} (${duration}ms)`,
			{
				component: 'http',
				requestId,
				metadata: {
		...
		MetricsCollector.recordHttpRequest(event.request.method, route, 500, duration);
